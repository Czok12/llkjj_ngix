...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if stats['errors'] > 0:
                raise CommandError(f"Import mit {stats['errors']} Fehlern abgeschlossen")
    ...
    def _simulate_import(self, konten_daten, force_update):
        ...
        stats = {'total': len(konten_daten), 'created': 0, 'updated': 0, 'skipped': 0, 'errors': 0}
        ...
        for konto_data in konten_daten:
            try:
                ...
                if exists:
                    if force_update:
                        stats['updated'] += 1
                        self.stdout.write(f"  📝 Würde aktualisiert: {nummer} - {konto_data['name']}")
                    else:
                        stats['skipped'] += 1
                        self.stdout.write(f"  ⏭️  Würde übersprungen: {nummer} - {konto_data['name']}")
                else:
                    stats['created'] += 1
                    self.stdout.write(f"  ✨ Würde erstellt: {nummer} - {konto_data['name']}")
                    
            except Exception as e:
                stats['errors'] += 1
                self.stdout.write(
                    self.style.ERROR(f"  ❌ Fehler bei {konto_data.get('nummer', 'UNBEKANNT')}: {str(e)}")
                )
        ...
        with transaction.atomic():
            for konto_data in konten_daten:
                try:
                    ...
                    if created:
                        stats['created'] += 1
                        self.stdout.write(f"  ✨ Erstellt: {konto.nummer} - {konto.name}")
                    elif force_update:
                        ...
                        konto.save()
                        stats['updated'] += 1
                        self.stdout.write(f"  📝 Aktualisiert: {konto.nummer} - {konto.name}")
                    else:
                        stats['skipped'] += 1
                        self.stdout.write(f"  ⏭️  Übersprungen: {konto.nummer} - {konto.name}")
                        
                except Exception as e:
                    stats['errors'] += 1
                    error_msg = f"Fehler bei {konto_data.get('nummer', 'UNBEKANNT')}: {str(e)}"
