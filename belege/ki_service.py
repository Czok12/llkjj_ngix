...
class BelegKategorisierungsKI:
    ...
    def kategorisiere_beleg(self, ocr_text: str, lieferant: str = None, betrag: float = None) -> Tuple[str, float]:
        """
        ...
        if lieferant:
            historie_kategorie, historie_vertrauen = self._pruefe_lieferanten_historie(lieferant)
            if historie_vertrauen > 0.8:
                logger.info(f"Lieferanten-Historie: {historie_kategorie} ({historie_vertrauen:.2f})")
        ...
        ml_kategorie, ml_vertrauen = self._ml_kategorisierung(ocr_text, lieferant, betrag)
        ...
        logger.info(f"Finale Kategorisierung: {finale_kategorie} ({finales_vertrauen:.2f})")
    ...
    def _lade_kategorien_regeln(self) -> Dict[str, List[str]]:
        ...
        return {
            'BÜROMATERIAL': [
                r'papier', r'stift', r'ordner', r'toner', r'drucker', r'bürobedarf',
                r'schreibwaren', r'kartusche', r'hefter', r'locher'
            ],
            'REISEKOSTEN': [
                r'hotel', r'bahn', r'flug', r'taxi', r'übernachtung', r'tankstelle',
                r'benzin', r'diesel', r'parken', r'maut', r'vignette'
            ],
            'MARKETING': [
                r'werbung', r'anzeige', r'google', r'facebook', r'instagram', r'adwords',
                r'seo', r'marketing', r'banner', r'plakat', r'flyer'
            ],
            'MIETE': [
                r'miete', r'nebenkosten', r'strom', r'gas', r'wasser', r'heizung',
                r'hausgeld', r'grundsteuer', r'müllgebühr'
            ],
            'VERSICHERUNG': [
                r'versicherung', r'prämie', r'police', r'schutz', r'haftpflicht',
                r'berufshaftpflicht', r'rechtsschutz'
            ],
            'WEITERBILDUNG': [
                r'seminar', r'kurs', r'workshop', r'schulung', r'fortbildung',
                r'weiterbildung', r'training', r'coaching'
            ],
            'RECHNUNG_EINGANG': [
                r'rechnung', r'invoice', r'bestellung', r'lieferung', r'material',
                r'dienstleistung', r'service'
            ],
            'BETRIEBSAUSGABE': [
                r'gebühr', r'abonnement', r'lizenz', r'software', r'tool',
                r'wartung', r'reparatur', r'instandhaltung'
            ]
        }
    ...
    def _pruefe_lieferanten_historie(self, lieferant: str) -> Tuple[str, float]:
        ...
        historie = BelegKategorieML.objects.filter(
            lieferant_name__icontains=lieferant
        ).values('korrekte_kategorie').annotate(count=Count('korrekte_kategorie')).order_by('-count')
    ...
    def _ml_kategorisierung(self, ocr_text: str, lieferant: str = None, betrag: float = None) -> Tuple[str, float]:
        """
    ...
    def _extrahiere_features(self, ocr_text: str, lieferant: str = None, betrag: float = None) -> Dict:
    ...
    def _berechne_aehnlichkeit(self, features: Dict, training_data: BelegKategorieML) -> float:
        ...
        try:
            ...
            gemeinsame_woerter = set(features['schluesselwoerter']) & set(training_woerter)
    ...
    def _kombiniere_ergebnisse(self, ergebnisse: List[Tuple[str, float]]) -> Tuple[str, float]:
        """
        ...
        finales_vertrauen = min(0.95, kategorien_votes[beste_kategorie] / len(ergebnisse))
    ...
    def _lade_lieferanten_historie(self) -> Dict[str, str]:
        ...
        belege = Beleg.objects.filter(
            benutzer_bestaetigt=True,
            geschaeftspartner__isnull=False
