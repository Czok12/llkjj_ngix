...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "pdf_pfad", type=str, help="Pfad zur PDF-Datei, die getestet werden soll"
        )
        ...
        parser.add_argument(
            "--save-text", type=str, help="Extrahierten Text in Datei speichern"
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS("🎯 Peter Zwegat's PDF-Extraktions-Test startet!")
        )
        ...
        try:
            ...
            for label, key in felder:
                wert = daten.get(key)
                if wert:
                    self.stdout.write(f"{label:20}: {self.style.SUCCESS(wert)}")
                else:
                    self.stdout.write(
