...
class BuchungsService:
    ...
    def importiere_csv_buchungen(
        ...
        with transaction.atomic():
            for i, zeile in enumerate(csv_daten, 1):
                try:
                    ...
                    soll_konto, haben_konto = BuchungsService._bestimme_konten_intelligent(
        ...
        logger.info(f"CSV-Import abgeschlossen: {erfolgreiche_importe} erfolgreich, {len(fehler)} Fehler")
    ...
    def _bestimme_konten_intelligent(
        ...
        elif any(keyword in text for keyword in ["lastschrift", "überweisung", "ausgang"]):
    ...
    def get_buchungs_statistiken(zeitraum_start=None, zeitraum_ende=None) -> Dict:
        ...
        return {
            ...
            "automatische_buchungen": queryset.filter(automatisch_erstellt=True).count(),
    ...
    def finde_aehnliche_buchungen(
        ...
        if buchung.betrag:
            ...
            aehnliche = aehnliche.filter(
                betrag__gte=min_betrag,
                betrag__lte=max_betrag
            )
...
class GeschaeftspartnerService:
    ...
    @staticmethod
    def finde_partner_by_name(name: str) -> Optional[Geschaeftspartner]:
        """
        ...
        # Teilübereinstimmung
        aehnliche = Geschaeftspartner.objects.filter(
